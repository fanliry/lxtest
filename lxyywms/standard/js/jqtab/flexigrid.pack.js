		...
		p = $.extend( {
			...
			__mw : {
				datacol : function(e, t, n) {
					var r = typeof e.datacol[t] == "function" ? e.datacol[t](n)
							: n;
		...
		var g = {
			...
			/**/addRowProp : function() {
				$("tbody tr", g.bDiv).on("click", function(e) {
					...
					//判断是否全选
					var checked = $("thead tr:first th:first-child", g.hDiv).children("div").children(":input").prop("checked");
					if(!checked){//没有全选有且只有一行选择
						var thisobj = this;
						//保证只有一个选中
						$("tbody tr", g.bDiv).each(function(index,obj){
							if(obj==thisobj){
			...
			for ( var i = 0; i < p.buttons.length; i++) {
				...
				if (!btn.separator) {
					...
					btnDiv.innerHTML = "<div><span>"
							+ (btn.hidename ? "聽" : btn.name) + "</span></div>";
					if (btn.bclass)
