...
function WebCalendar() //初始化日历的设置
{
      ...
      this.todayColor = "#0000FF";      //今天在日历上的标示背景色
...
//    "      onclick=\"parent.returnDate(new Date().getDate() +'/'+ (new Date().getMonth() +1) +'/'+ new Date().getFullYear())\">"+
	  "      onclick=\"parent.returnDate('today')\">"+
...
function writeCalendar() //对日历显示的数据的处理程序
{
      ...
      {
          var a = WebCalendar.day[i].toString().split("/");
...
function returnDate(flg) //根据日期格式等返回用户选定的日期
{
      if(WebCalendar.objExport)
      {
          var returnValue;
          //var a = (arguments.length==0) ? WebCalendar.day[this.id.substr(8)].split("/") : arguments[0].toString().split("/");
          var tmpdate = new Date().getDate() +'/'+ (new Date().getMonth() +1) +'/'+ new Date().getFullYear();
		  var a = (flg=="today") ? tmpdate.split("/") : WebCalendar.day[this.id.substr(8)].split("/");
