...
<body onLoad="OnLoad()">
 <table width="100%" height="100%"   border="0" align="center" cellpadding="0" cellspacing="0" class="TABLE_LIST">
   <tr>
     ...
     <td class="TD_LIST_TITLE2"><div class="list_title">是否上传</div></td>
   </tr>
<%
	...
	if(ls != null && ls.size() > 0) 
	{
		...
		String instockid;      	//入库单据编号
     	String instatus;       	//单据状态
     	String intype;         	//入库类型
     	String createman;    	//单据生成人员
     	String createtime;     	//单据生成时间
     	String uploadflag;     	//上传标志0-已上传（默认）1-未上传
     	String invoicesource;  	//单据来源

		for(int i = 0; i < ls.size(); i++)
		{
			inBound = (InboundHeader)ls.get(i);
			instockid = inBound.getInstockid();   		//入库单据编号
	        intype = inBound.getInvoicetypename();   	//入库类型
	        instatus = inBound.getInstatusname();		//单据名称
	        createman = inBound.getCreatemanid();		//单据生成人员编号
	        createtime = inBound.getCreatetime();   	//单据生成时间
         	uploadflag = (inBound.getUploadflag()==null || inBound.getUploadflag().equals("N")) ?"N":"Y";    //上传标志0-已上传（默认）1-未上传
