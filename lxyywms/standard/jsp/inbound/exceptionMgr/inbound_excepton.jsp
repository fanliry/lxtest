...
<%
	...
	boolean inboundExceptQuery = false;    
	boolean inboundExceptClean = false;     
	boolean inboundExceptUploading = false;     
	
	if(request.getSession(false).getAttribute("hsPopedom") != null){
		hsPopedom = (HashMap)request.getSession(false).getAttribute("hsPopedom");
	}
	if(hsPopedom != null){
		if(hsPopedom.get("inboundExceptQuery") != null){
			inboundExceptQuery = true;
		}
		if(hsPopedom.get("inboundExceptClean") != null){
			inboundExceptClean = true;
		}
		if(hsPopedom.get("inboundExceptUploading") != null){
			inboundExceptUploading = true;
		}
    }
%>
<%
...
<!--
	...
	function searchData(){
		...
		list.formx1.action = strUrl + "NeedCheckAction&flag=1" + condition;
    ...
    <tr><td>
	  ...
	  <div class="wz">
		...
		<div id="caozuo" class="f_r">
		  <ul>
		    <%if(inboundExceptClean){%><li class="tubiao1"  style="width:90px"><a href="#" onclick="clearspace()">清空货位</a></li><%}%>
			<%if(inboundExceptQuery){%><li class="tubiao1"><a href="#" onclick="searchData()">查询</a></li><%}%>
		  </ul>
		</div>
	  </div>
    ...
    <tr>
      <td valign="top" height="100%">
		...
		<table width="99%" height="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		  <tr>
		    <td>
	          <iframe id="list" src="<%=request.getContextPath()%>/standard/jsp/inbound/exceptionMgr/inbound_excepton_list.jsp" 
