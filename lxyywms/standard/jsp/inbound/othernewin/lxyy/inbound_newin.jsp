...
<%
	...
	boolean inbound_otnewrkdquery = false;    //查询
	boolean inbound_otnewrkdsave = false;     //生成入库单据
	
	if(request.getSession(false).getAttribute("hsPopedom") != null){
		hsPopedom = (HashMap)request.getSession(false).getAttribute("hsPopedom");
	}
	if(hsPopedom != null){
		if(hsPopedom.get("inbound_otnewrkdquery") != null){
			inbound_otnewrkdquery = true;
		}
		if(hsPopedom.get("inbound_otnewrkdsave") != null){
			inbound_otnewrkdsave = true;
		}
    }
%>
<%
...
<!--
	...
	function searchData(){
		...
		list.location.href = strUrl + "OtherInBoundAction&flag=1" + condition;
	}
	...
	function saveData(){
		...
		condition =  keys.substring(0, keys.length-1)+"&warehouseid=" +　warehouseid + "&lotinfo=" + lotinfo + "&starttime=" + starttime + "&endtime=" +　endtime
			 + "&instockid=" + instockid + "&isform=" + isform + "&invoicetype=" + invoicetype;
		list.location.href = strUrl + "OtherInBoundAction&method=ricoExecCreate&condition="+condition;			
	}
...
<div class="con_bk">
  <table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
    <tr><td>
	  ...
	  <!-- ======== 当前位置 ======== -->
	  <div class="wz">
		<div id="dqwz" class="f_l">当前位置：<span>入库管理 &gt;&gt; 其他入库管理 &gt;&gt; 新建入库单</span></div>
		<div id="caozuo" class="f_r">
		  <ul>
		  	<%if(inbound_otnewrkdsave){%><li class="tubiao4" style="width:100px;"><a href="#" onclick="saveData();">生成入库单据</a></li><%}%>
			<%if(inbound_otnewrkdquery){%><li class="tubiao1"><a href="#" onclick="searchData()">查询</a></li><%}%>
		  </ul>
		</div>
	  </div>
    ...
    <tr><td>
      ...
      <table width="99%"  border="0" cellpadding="0" cellspacing="0" class="table1" align="center"> 
	    ...
	    <tr>
          ...
          <td class="x">
              <select id="invoicetype" style="width:110px;">
	      ...
	      <tr>
	        <td height="180px">
		      <iframe id="list" width="100%" height="100%" frameborder="0" scrolling="yes"
	               src="<%=request.getContextPath()%>/standard/jsp/inbound/othernewin/fyhz/inbound_newin_list.jsp"></iframe>
		    </td>
	      </tr>
	      <tr>
	       <td height="25">
