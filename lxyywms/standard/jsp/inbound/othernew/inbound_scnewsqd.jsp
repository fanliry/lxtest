...
<%
	...
	boolean inbound_otnewsqdproduct = false;    //选择产品
	boolean inbound_otnewsqdsave = false;     //保存
	
	if(request.getSession(false).getAttribute("hsPopedom") != null){
		hsPopedom = (HashMap)request.getSession(false).getAttribute("hsPopedom");
	}
	if(hsPopedom != null){
		if(hsPopedom.get("inbound_otnewsqdproduct") != null){
			inbound_otnewsqdproduct = true;
		}
		if(hsPopedom.get("inbound_otnewsqdsave") != null){
			inbound_otnewsqdsave = true;
		}
    }
%>
<%
...
<!--自适应高度的div-->
	...
	function add()
	{
		...
		if(result.length > 0)
		{
			list.location.href="<%=request.getContextPath()%>/BMSService?actionCode=OtherInBoundRequestAction&method=ricoExecCreate" + url;
		}
		else
		{
			alert("请选择货物!");
...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
   <tr>
     <td>
	     <div class="wz" >
	      <div id="dqwz" class="f_l">当前位置：<span>入库管理</span> &gt;&gt;<span>其他入库管理</span> &gt;&gt; <span>新建申请单</span></div>
	      <div  class="f_r" id="caozuo">
	        <ul>
	          <%if(inbound_otnewsqdsave){%><li class="tubiao1"><a href="#" onclick="add();">保存</a></li><%}%>
	          <%if(inbound_otnewsqdproduct){%><li class="tubiao4" style="width:90px"><a href="#" onclick="Select();">选择产品</a></li><%}%>
	        </ul>
	      </div>
	    </div>
 ...
 <table id="querycondition" width="99%"  border="0" align="center" cellpadding="0" cellspacing="0" class="table1">
              <tr>
                ...
                <td class="xx1"  ><div align="left">
                  <input name="invoicedate" type="text" size="13" value="<%=strTime%>" onfocus="calendar();"  class="Wdate" style="height:18px;width:100px;" />
                </td>
             </tr>
             <tr>
                ...
                <td width="200" class="x" >
                	<input type="text" name="remark"   style="height:18px;width:100px;"/>
                </td>  
                <td></td>
                <td></td> 
              </tr> 
	   ...
	   <tr>
	     <td height="180">
		   <iframe id="list" name="list" width="100%" height="100%"   frameborder="0"  scrolling="yes"
	               src="<%=request.getContextPath()%>/standard/jsp/inbound/othernew/inbound_scnewsqd_list.jsp"></iframe>
		 </td>
	   </tr>
	 </table>
     </td>
