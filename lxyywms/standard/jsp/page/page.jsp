...
<!-- 
	...
	function ToPage(page){
		if(page != 0){
			...
			parent.list.location.href= ac + "commonPaging&page=" + page;
		}
		else{
			...
			if(IsNum(page) && 0 < page && page <= max_page){
				...
				var isquery = document.getElementById("isQueryFlag").value;
				if(isquery ==0 ){ //是否有查询内容  没有时
...
<%
	int iMaxRow = 50;
	String strMaxRow = request.getParameter("linerow");
	...
	int iCurrentPage = 1;
	int iCountPage = 1;
	PagingTool pt = (PagingTool)request.getSession(false).getAttribute("paging");
	int isQueryFlag = 0; //是否有查询内容
	if(pt != null){
	    isQueryFlag =1;
...
<input name="isQueryFlag" type="hidden" value="<%=isQueryFlag%>">
