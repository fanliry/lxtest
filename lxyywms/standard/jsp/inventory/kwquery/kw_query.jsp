...
<%
	...
	boolean kcKwqueryQuery = false; //查询
	
	if(request.getSession(false).getAttribute("hsPopedom") != null){
		hsPopedom = (HashMap)request.getSession(false).getAttribute("hsPopedom");
	}
	if(hsPopedom != null){
		if(hsPopedom.get("kcKwqueryQuery") != null){
			kcKwqueryQuery = true;
		}
    }
%>
...
<!--
	...
	function Search(){
		...
		condition = "&warehouseid=" + warehouseid + "&whAreaId=" + whAreaId + "&cargoAreaId=" + cargoAreaId + "&platoon=" + platoon 
				+ "&column=" + column + "&floor=" + floor + "&in_lock=" + in_lock + "&out_lock=" + out_lock + "&csstatus=" + csstatus+ "&linerow=" + linerow;
		Loading();
 ...
 <div class="con_bk">
   <table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
    <tr><td>
	  ...
	  <!-- ======== 当前位置 ======== -->
	  <div class="wz">
		<div id="dqwz" class="f_l">当前位置：<span>库存管理 &gt;&gt; 库位查询</span></div>
		<div id="caozuo" class="f_r">
		  <ul>
			<%if(kcKwqueryQuery){%><li class="tubiao1"><a href="#" onclick="Search()">查询</a></li><%}%>
		  </ul>
		</div>
	  </div>
    ...
    <tr>
      <td valign="top" height="100%">
		  ...
		  <tr>
		    <td>
	          <iframe id="list" src="<%=request.getContextPath()%>/standard/jsp/inventory/kwquery/kw_query_list.jsp"
		  		width="100%" height="100%" scrolling="auto" frameborder="0"></iframe>
		  	</td>
		  </tr>
		  <tr>
		    <td height="25">
		       <iframe  id="page" name="page" src="<%=request.getContextPath()%>/standard/jsp/page/page.jsp?linerow=20&flag=N"   width="100%" height="100%" scrolling="no"  frameborder="0" ></iframe> 
		    </td>
		  </tr>
		</table>
