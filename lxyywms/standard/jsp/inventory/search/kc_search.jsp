...
<%
	...
	boolean kcCheckqueryQuery = false; //查询
	boolean kcCheckqueryReport = false; //报表
	
	if(request.getSession(false).getAttribute("hsPopedom") != null){
		hsPopedom = (HashMap)request.getSession(false).getAttribute("hsPopedom");
	}
	if(hsPopedom != null){
		if(hsPopedom.get("kcCheckqueryQuery") != null){
			kcCheckqueryQuery = true;
		}
		if(hsPopedom.get("kcCheckqueryReport") != null){
			kcCheckqueryReport = true;
		}
    }
%>
<html>
...
<!--
	...
	function searchData(){
		...
		var producttype = document.getElementById("producttype").value;		//产品类别
		...
		condition = "&warehouseid=" + warehouseid + "&whAreaId=" + whAreaId + "&Virtualwhid=" + Virtualwhid + "&cargoAlleyId=" + cargoAlleyId + "&platoon=" + platoon + 
			"&column=" + column + "&floor=" + floor + "&lotinfo=" + lotinfo + "&productid=" + package_id + "&tray_code=" + tray_code+ "&producttype=" +producttype+ "&productstatus=" +productstatus+ "&status=" +csstatus+ "&indate_from=" +indate_from+ "&indate_to=" +indate_to;
...
<div class="con_bk">
  <table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
    <tr><td>
	  ...
	  <!-- ======== 当前位置 ======== -->
	  <div class="wz">
		<div id="dqwz" class="f_l">当前位置：<span>库存管理 &gt;&gt; 库存查询</span></div>
		<div id="caozuo" class="f_r">
		  <ul>
		    <%if(kcCheckqueryReport){%><li class="tubiao2"><a href="#" onClick="report()">报表</a></li><%}%>
			<%if(kcCheckqueryQuery){%><li class="tubiao1"><a href="#" onclick="searchData()">查询</a></li><%}%>
		  </ul>
		</div>
	  </div>
    ...
    <tr><td>
      ...
      <table id="marginTable" width="99%"  border="0" cellpadding="0" cellspacing="0" class="table1" align="center"> 
	    ...
	    <tr>
	      ...
	      <td class="yx">
	      	<select id="csstatus" style="width:117px;">
    ...
    <tr>
      <td valign="top" height="100%">
		...
		<table width="99%" height="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		  <tr>
		    <td>
	          <iframe id="list" src="<%=request.getContextPath()%>/standard/jsp/inventory/search/kc_search_list.jsp" 
