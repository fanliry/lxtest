...
<div style="width: 100%; height: 100%;overflow-x:scroll; overflow-y:auto; position:absolute;">
  <table width="120%" id="theTable" height="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="TABLE_LIST">
   <tr>
     ...
     <td class="TD_LIST_TITLE"><div class="list_title">有效期</div></td>
     <td class="TD_LIST_TITLE"><div class="list_title">到过期</div></td>
...
<%
	...
	if(ls2 != null){
		...
		String productname2;			// 物品名2
	
	    double validityterm;		// 有效期
	    String validitytypename;	// 有效期类型名
	
		
		//把产品  以形式（物品名，有效期）放入集合          注：有效期类型 默认为天。
		...
		for(int i=0; i<ls2.size(); i++){
			...
			productname2 = product.getProductname();		// 物品名2
        ...
        String productstatus;     //物品状态
  	 	for(int i=0; i<ls.size(); i++) 
  	 	{
			...
			indate = storage.getIndate();       //入库时间
  	 		...
  	 		losdate = treeMap.get(product);  //有效期
  	 		
  	 		warningdate = isWarningDate(printdate , losdate);  //剩余时间
...
%>
   <tr onmouseover="this.bgColor='#E3F2FF'" onmouseout="this.bgColor=''">
     ...
     <td class="TD_LIST" align="center"><%=losdate%>天</td>
     
     <td class="TD_LIST" align="center">
    <%      
    out.println("<span style=\"color:" +(warningdate < 50 ? "red;font-weight: bold" : "black") + ";\">" + warningdate +"天"+ "</span>");
    %>
    </td>
