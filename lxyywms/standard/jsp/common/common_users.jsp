...
<!--
	...
	function queryData()
	{
		...
		condition = "&userCode=" + userCode + "&userName=" + userName + "&unitCode=" + unitCode+ "&usersflag=" + usersflag + "&linerow="+linerow;
		list.location.href = ac + "UserAction" + condition;
	}

	function OnOk()
	{   
		 var check_ids = window.list.document.getElementsByName("check_id");
		 var strPram = "";
		 if (check_ids==null)
		 {
			  window.close();
		 ...
		 if(strPram != null){
		   strPram = strPram.substring(0, strPram.length-1);
		 }
	    ...
	    <tr>
	      <td>
	 	    <iframe id="list" name="list" width="100%" height="100%" frameborder="0" scrolling="auto"
				src="<%=request.getContextPath()%>/standard/jsp/common/common_users_list.jsp"></iframe>
	      </td>
	    </tr> 
 	  </table>
