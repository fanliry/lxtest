...
<%
	...
	boolean outboundFhqrQuery = false; 
	boolean outboundFhqrAdjust = false;
	boolean outboundFhqrD = false; 
	boolean outboundFhqr = false; 
	
	if(request.getSession(false).getAttribute("hsPopedom") != null){
		hsPopedom = (HashMap)request.getSession(false).getAttribute("hsPopedom");
	}
	if(hsPopedom != null){
		if(hsPopedom.get("outboundFhqrQuery") != null){
			outboundFhqrQuery = true;
		}
		if(hsPopedom.get("outboundFhqrAdjust") != null){
			outboundFhqrAdjust = true;
		}
		if(hsPopedom.get("outboundFhqrD") != null){
			outboundFhqrD = true;
		}
		if(hsPopedom.get("outboundFhqr") != null){
			outboundFhqr = true;
		}
    }
%>
...
<!--
	...
	{ 
		...
		var check_msg = "";
		DWREngine.setAsync(false);
		judgmentTool.sendCheck1(warehouse_out_id, ShowCheck);
		DWREngine.setAsync(true);
		...
		if (check_msg == "Y") {
		        ...
		        DWREngine.setAsync(false);
				judgmentTool.sendCheck2(warehouse_out_id, ShowCheck2);
				DWREngine.setAsync(true);
				function ShowCheck2(data) {
					check_msg = data;
				}
			...
			msg = "单据【" + warehouse_out_id + "】的这条单据，因为以下检测未通过，不可以发货确认！\r\r";
			msg += check_msg ;
			alert(msg);
		}
	}
	...
	{ 
		...
		var check_msg = "";
		DWREngine.setAsync(false);
		judgmentTool.sendOneCheck1(warehouse_out_id, ids, ShowCheck);
		DWREngine.setAsync(true);
		...
		if (check_msg == "Y") {
			 ...
			 DWREngine.setAsync(false);
			 judgmentTool.sendOneCheck2(warehouse_out_id, ids, ShowCheck2);
			 DWREngine.setAsync(true);
			...
			msg = "单据【" + warehouse_out_id + "】的这条单据详细，因为以下检测未通过，不可以发货确认！\r\r";
			msg += check_msg ;
			alert(msg);
		}
...
//出库调整
	...
	function adjust()
	{
		...
		if(outinvoiceid == null || outinvoiceid.length < 1){
			...
			var result = showModalDialog("<%=request.getContextPath()%>/standard/jsp/outbound/fhqr/outbound_fhqr_adjust.jsp?outid="+outinvoiceid+"&warehouseid="+warehouseid, 
						outinvoiceid, "dialogWidth=900px; dialogHeight=600px; scroll=no");
    ...
    <tr><td>
	  ...
	  <div class="wz">
		...
		<div id="caozuo" class="f_r">
		  <ul>
          	<%if(outboundFhqr){%><li class="tubiao1" style="width:90px;"><a href="#" onclick="sendOk();">发货确认</a></li><%}%>
		    <%if(outboundFhqrD){%><li class="tubiao2" style="width:110px;" ><a href="#" onclick="sendOneOk();">按详细发货确认</a></li><%}%>
		    <%if(outboundFhqrAdjust){%><li class="tubiao2" style="width:90px;" ><a href="#" onclick="adjust();">发货调整</a></li><%}%>
		    <%if(outboundFhqrQuery){%><li class="tubiao2" style="width:90px;" ><a href="#" onclick="sendCheck();">发货检测</a></li><%}%>
		  </ul>
		</div>
	  </div>
