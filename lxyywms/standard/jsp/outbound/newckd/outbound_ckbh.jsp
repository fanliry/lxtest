...
<%
	...
	boolean outboundStockQuery = false; 
	boolean outboundStockCreate = false; 
	boolean outboundStockAssign = false; 
	boolean outboundStockQueryD = false; 
	boolean outboundStockUploading = false; 
	
	if(request.getSession(false).getAttribute("hsPopedom") != null){
		hsPopedom = (HashMap)request.getSession(false).getAttribute("hsPopedom");
	}
	if(hsPopedom != null){
		if(hsPopedom.get("outboundStockQuery") != null){
			outboundStockQuery = true;
		}
		if(hsPopedom.get("outboundStockCreate") != null){
			outboundStockCreate = true;
		}
		if(hsPopedom.get("outboundStockAssign") != null){
			outboundStockAssign = true;
		}
		if(hsPopedom.get("outboundStockQueryD") != null){
			outboundStockQueryD = true;
		}
		if(hsPopedom.get("outboundStockUploading") != null){
			outboundStockUploading = true;
		}
    }
%>
<%
...
<!--自适应高度的div-->
 	...
 	function addinvoicedetail()
 	{
		...
		if( k == 0 ){
 		...
 		showModalDialog("<%=request.getContextPath()%>/standard/jsp/outbound/newckd/outbound_newckd_detail.jsp?invoiceid="+id,"new","dialogWidth=800px;dialogHeight=600px;");
	...
	//分配
 	function assign()
 	{
		...
		var djid = document.getElementById("dckdid").innerHTML;
		list.queryDetail(djid);
 	}
 	//按批号拣货出库
 	function lotinfopick()
 	{    		
		var WLeft = Math.ceil((window.screen.width-1100)/2);
  		var WTop  = Math.ceil((window.screen.height-800)/2);
		showModalDialog("<%=request.getContextPath()%>/standard/jsp/outbound/lotinfopick/lotinfopick_assgin.jsp",'newwindow',"dialogWidth=500px;dialogHeight=600px;");
 	}
 	//查询
	...
	{	
	    ...
	    var WLeft = Math.ceil((window.screen.width-900)/2);
  		var WTop  = Math.ceil((window.screen.height-400)/2);
  	    showModalDialog("<%=request.getContextPath()%>/standard/jsp/outbound/ckdmx/outbound_ckdDetail.jsp?&instockid="+id,"new","dialogLeft='"+WLeft+"';dialogTop='"+WTop+"';dialogWidth=900px;dialogHeight=400px;");
	}
...
<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0">
   <tr>
     <td>
	     ...
	     <div class="wz" >
	      <div id="dqwz" class="f_l">当前位置：<span>出库管理</span> &gt;&gt; <span>出库备货</span></div>
	      <div  class="f_r" id="caozuo">
	        <ul>
	           <%if(outboundStockQueryD){%><li class="tubiao4" style="width:140px;"><a href="#" onclick="disaplayReceive();">出库单分配明细显示</a></li><%}%>
	           <%if(outboundStockAssign){%><li class="tubiao2"><a href="#" onclick="assign();">分配</a></li><%}%>
	           <%if(outboundStockCreate){%><li class="tubiao2"  style="width:110px;"><a href="#" onclick="lotinfopick();">按批号拣货出库</a></li><%}%>
	           <%if(outboundStockQuery){%><li class="tubiao1"><a href="#" onclick="queryinvoice();">查询</a></li><%}%>
	        </ul>
	      </div>
	    </div>
