	...
	function Getrequestvoiceid(){
		...
		if(returnValue != null){
			...
			if(Rearr.length==9){
				...
				if("null" != Rearr[1]){
					document.getElementById("podetailid").value = Rearr[1]; // PO行号				
				}
				if("null" != Rearr[2]){
					document.getElementById("productid").value = Rearr[2]; // 产品ID 
				}
				if("null" != Rearr[3]){
					document.getElementById("productName").value = Rearr[3]; // 产品名称
				}
				if("null" != Rearr[4]){
					document.getElementById("productCode").value = Rearr[4]; // 产品编码
				}
				if("null" != Rearr[5]){
					document.getElementById("punit").value = Rearr[5]; // eunitName 单位
				}
				if("null" != Rearr[6]){
					if(document.getElementById("lotinfo")){
						document.getElementById("lotinfo").value = Rearr[6]; // lotinfo 进场编号
					}
				}
				if("null" != Rearr[7]){
					if(document.getElementById("lotinfo2")){
						document.getElementById("lotinfo2").value = Rearr[7]; // lotinfo2 原厂编号
					}
				}
				document.getElementById("jobnum").value = Rearr[8]; // ponum 数量
			}else{
				alert(Rearr.length);
			}		
...
/* 		if(printdate == null || printdate.length < 1){
		...
		condition = "&poid=" + poid + "&podetailid=" + podetailid + "&warehouseid=" + warehouseidzz + "&whAreaId=" + whAreaId + "&intype=" + intype 
				...
				+ "&productstatus=" + productstatus + "&lotinfo=" + lotinfo + "&jobnum=" + jobnum 
				+ "&snumber=" + snumber	+ "&traycode=" + traycode + "&printdate=" + printdate 
				+"&productCode=" + productCode+ "&Virtualwhid=" + Virtualwhid;

	    if(confirm("你确定收货吗？"))
		{		
			window.location.href = strUrl + "inBoundJobAction&method=ricoExecCreateRFJob&rfmainView=rfmainView&flag=tlrk"+ condition;
		}
...
<body onload="onLoad();">
	<table id="tbb" border="1" align="center" cellpadding="0" cellspacing="0" class="TABLE_MGR_TOP">
		<tr>
			<td height="25" bgcolor="#d9e8fc" align="center" colspan="2">
				<table width="100%">
					<tr>
						<td width="20%"></td>
						<td width="60%" align="center" class="font_006699_bold_12">RF退料入库</td>
		...
		<tr>  
	     <td class="TD_ADD" align="right">PO行号：</td>
	     <td class="TD_ADD">
		...
		<tr>
			<td class="TD_ADD" align="right">逻辑库区：</td>
     	  	...
     	  	<select id="Virtualwhid" style="width:163px;" onchange="GetWhInfo();" ><option value=""></option><font color="red"> *</font></select>
     	  	</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="TD_ADD">
			<select id="snumber" style="width:163px;">
                <option value="">--请选择--</option>
