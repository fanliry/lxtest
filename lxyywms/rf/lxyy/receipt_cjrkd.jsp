...
<script type="text/javascript">
	...
	function Getrequestvoiceid(){
		...
		if(returnValue != null){
			...
			if(Rearr.length==9){
				...
				document.getElementById("productid").value = Rearr[2]; // 
				document.getElementById("productName").value = Rearr[3]; // 
				document.getElementById("productCode").value = Rearr[4]; // 
				document.getElementById("punit").value = Rearr[5]; // eunitName 单位
				if(Rearr[6] != null && Rearr[6].length > 0){
					if(document.getElementById("lotinfo")){
						document.getElementById("lotinfo").value = Rearr[6]; // 
					}
				}
				if(Rearr[7] != null && Rearr[7].length > 0){
					if(document.getElementById("lotinfo2")){
						document.getElementById("lotinfo2").value = Rearr[7]; // 
					}
				}
	...
	function saveData()
	{
	   ...
	   if(warehouseid=null || warehouseid==""){
		...
		condition = "&poid=" + poid + "&podetailid=" + podetailid + "&warehouseid=" + warehouseidzz + "&whAreaId=" + whAreaId + "&intype=" + intype 
				...
				+"&productCode=" + productCode + "&supplier=" + supplier;

	    if(confirm("你确定收货吗？"))
		{		
			window.location.href = strUrl + "inBoundJobAction&method=ricoExecCreateJob&rfmainView=rfmainView"+ condition;
		}
	...
	function onLoad()
	{
		...
		selectType('14', 'intype', 'rklx');			//入库类型
...
<body onload="onLoad();">
	<table id="tbb" border="1" align="center" cellpadding="0" cellspacing="0" class="TABLE_MGR_TOP">
		<tr>
			<td height="25" bgcolor="#d9e8fc" align="center" colspan="2">
				<table width="100%">
					<tr>
						<td width="20%"></td>
						<td width="60%" align="center" class="font_006699_bold_12">RF抽检入库</td>
		...
		<tr>
			...
			<td class="TD_ADD"><input type="text" name="lotinfo2" class="rf_input_long" maxlength="10" > <font color="red">*</font></td>
		</tr>
		<tr>
